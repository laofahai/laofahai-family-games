...
function uniqueNumbers(answer: number, min: number, max: number, preferred: number[] = []): number[] {
...
function numberChoices(answer: number, unit: string, min = 0, max = 40, preferred: number[] = []) {
  return uniqueNumbers(answer, min, max, preferred).map((value) => c(String(value), `${value} ${unit}`))
}

function toJiao(yuan: number): number {
  return Math.round(yuan * 10)
}

function fromJiao(jiao: number): number {
  return jiao / 10
}
...
function moneyExpr(left: number, operator: '+' | '-', right: number, answer: number): string {
  return `${formatMoney(left)} ${operator} ${formatMoney(right)} = ${formatMoney(answer)}`
}

function moneyChoices(answer: number, preferred: number[] = []) {
...
function operationChoices(answerId: string, correctText: string, wrongTexts: string[]) {
