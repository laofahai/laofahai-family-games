...
import { Hero, HERO_MAX_HP } from './Hero'
...
export class ArenaScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // 主角（头顶挂玩家名）出生在近端 heroStartX。
    // TODO 性别区分待女版 Kenney 素材：现在不按性别换精灵，主角统一用 hero。
    this.hero = new Hero(this, this.stage.heroStartX, this.groundY, this.playerName)
  ...
  private resolveBossQuiz(correct: boolean): void {
    ...
    if (correct) {
      ...
      this.shockwave(boss.x, boss.y - boss.displayHeight * 0.5, color) // 按科目配色冲击波
      this.shieldShatter(boss) // 碎盾爆裂（一圈碎片四射）
      this.meleeReadyPulse(boss) // 金色「可揍」脉冲环
      ...
      playSfx('skill')
      playSfx('correct')
      this.bridge.emit('result', { ok: true, crit: true, title: '答对！护盾破了，快揍他', detail: cry ?? undefined })
    } else {
      // 答错：BOSS 反打主角（护盾仍在）。
  ...
  /** 碎盾爆裂：一圈蓝白碎片四射（破盾的纯视觉表演）。 */
  private shieldShatter(boss: Enemy): void {
    const cx = boss.x
    const cy = boss.y - boss.displayHeight * 0.5
    for (let i = 0; i < 18; i++) {
      const ang = (i / 18) * Math.PI * 2
      const spd = Phaser.Math.Between(180, 360)
      const shard = this.add.rectangle(cx, cy, Phaser.Math.Between(6, 12), 3, 0x9fd0ff, 1).setDepth(84).setRotation(ang)
      this.tweens.add({
        targets: shard,
        x: cx + Math.cos(ang) * spd,
        y: cy + Math.sin(ang) * spd,
        alpha: 0, scale: 0.3,
    ...
    const ring = this.add.circle(cx, cy, boss.displayHeight * 0.5, 0x9fd0ff, 0).setDepth(83)
    ring.setStrokeStyle(6, 0xcfe8ff, 0.9)
    this.tweens.add({ targets: ring, radius: boss.displayHeight * 0.95, alpha: 0, duration: 320, ease: 'Cubic.easeOut', onComplete: () => ring.destroy() })
  }

  /** 「可揍」脉冲：Boss 头顶冒一圈金色脉冲环（提示破盾窗口可近战，纯视觉、自动消失）。 */
  private meleeReadyPulse(boss: Enemy): void {
    ...
    for (let i = 0; i < 2; i++) {
      this.time.delayedCall(i * 180, () => {
        if (boss.dead) return
        const ring = this.add.circle(boss.x, boss.y - boss.displayHeight * 0.5, 18, 0xffd23f, 0).setDepth(82)
        ring.setStrokeStyle(5, 0xffe08a, 0.95)
        this.tweens.add({ targets: ring, radius: boss.displayHeight * 0.7, alpha: 0, duration: 460, ease: 'Cubic.easeOut', onComplete: () => ring.destroy() })
      })
    }
    // 「可揍」金字（短促自动飘走）。
    this.floatText(cx, cy - boss.displayHeight * 0.3, '可揍!', '#ffe08a', 22)
  }
  ...
  private onResize(): void {
    ...
    // 防抖：视口尺寸没变就早退（避免重画背景导致装饰抖动 / 无谓重建）。
